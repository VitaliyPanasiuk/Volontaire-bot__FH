...
products_for_pets_router = Router()


@products_for_pets_router.callback_query(lambda c: c.data == 'products for pets')
async def user_start(callback_query: types.CallbackQuery):
    userid = callback_query.from_user.id
    await user_update.update_action(userid, 'products for pets')
